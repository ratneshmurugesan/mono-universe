    ...
    return (
        <tr>
            {column.map((columnItem) => {
                ...
                if (columnItem["heading"] === 'checkbox') return <StyledTd><input type='checkbox' checked={rowObj["selected"]} onChange={(e) => handleOnRowSelect(e, rowObj["id"])} /></StyledTd>
                if (columnItem["heading"] === 'thumbnail') return <StyledTd><img src={rowObj["thumbnailUrl"]} alt={rowObj["title"]} width='50px' /></StyledTd>
                return <StyledTd align={align} onClick={() => handleOnRowClick(rowObj, rowObj["id"])}>
